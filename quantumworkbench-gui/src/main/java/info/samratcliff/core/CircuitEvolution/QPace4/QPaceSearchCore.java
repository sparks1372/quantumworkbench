...
import info.samratcliff.core.CircuitEvolution.SearchEngineCore;
    ...
    public static ParameterDatabase loadParameterDatabase(String[] args) {
        ...
        for (int x = 0; x < args.length - 1; x++) {
            if (args[x].equals(A_FILE)) {
                try {
                    System.out
                            .println("Loading parameters from " + args[x + 1]);
                    if (args[x + 1].startsWith("/"))
                        parameters = new ParameterDatabase(new File(args[x + 1]).getAbsoluteFile());
                    else
                        parameters = new ParameterDatabase(QPaceSearchCore.class.getClassLoader().getResourceAsStream(args[x + 1]));
